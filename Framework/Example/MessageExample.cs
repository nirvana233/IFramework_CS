...
namespace Example
{
    ...
    public class MessageExample : Test
    {
        ...
        public class Listener : IMessageListener
        {
            ...
            {
                Log.L(string.Format($"收到类型为{message.subject}的消息，消息值为{message.code}"));
            }
        }
        ...
        private void SendMessage()
        {
            for (int i = 0; i < 3; i++)
            {
                messageModule.Publish(this, null, MessageUrgencyType.Common).SetCode(i + 1);
            }
            Log.L("已发送紧急类型为普通的三条消息");
            for (int i = 0; i < 3; i++)
            {
                messageModule.Publish<MessageExample>(null,MessageUrgencyType.Unimportant).SetCode(i + 4);
            }
